...
public sealed partial class CpuFreqProvider : ISensorProvider
{
    private readonly string _cpuRoot;

    public CpuFreqProvider(string? cpuRoot = null)
    {
    ...
    public async Task<IReadOnlyCollection<SensorReading>> GetSensorReadingsAsync(CancellationToken cancellationToken)
    {
        ...
        foreach (var cpuDirectory in Directory.EnumerateDirectories(_cpuRoot, "cpu*"))
        {
            ...
            sensors.Add(SensorFactory.Create(
                sensorId,
                GroupPath.From("CPU", "Clocks", directoryName.ToUpperInvariant()),
                friendlyName,
                SensorType.CpuFrequency,
                mhz,
                "MHz",
                Name));
        }
